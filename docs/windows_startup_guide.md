...
1. **必须用 `serve.py` 启动**，它会加载 `.env`，连到 NAS 上唯一共享的 PostgreSQL 与对象存储。
2. 办公室电脑和家庭电脑只运行应用代码；**禁止**运行 PostgreSQL 容器、SQLite 或其他本地数据库。
3. **禁止**裸跑 `python -m uvicorn app.main:create_app ...`。缺少 NAS 配置、配置为 SQLite，或 NAS PostgreSQL 不可连接时，服务必须直接报错并停止。
4. 家庭电脑须先连接 VPN 或其他安全网络，确保能访问 NAS PostgreSQL 与对象存储端点。

## 二、标准启动步骤

```powershell
# 1) 确认根目录 .env 已填写 NAS PostgreSQL 与对象存储凭据
...
| 任务名 | 作用 |
| --- | --- |
| `file-asset-console` | 登录时用系统 Python 运行 `serve.py`（加载 `.env`，验证 NAS 数据库后起 uvicorn） |
...
```powershell
# 应返回 NAS PostgreSQL 连接串（不是 sqlite）以及 NAS MinIO endpoint
...
# 使用 NAS PostgreSQL 连接串检查档案数量（非 0/个位数）
psql $env:FILE_ASSET_DATABASE_URL -c "SELECT count(*) FROM archive WHERE domain_type='cost_info';"
```

若档案数异常偏小，检查 `.env` 是否指向 NAS；应用不会再回落到 SQLite 或本地 PostgreSQL。
...
- 因此"是否落 NAS"完全由**服务实例的 `.env`** 决定；缺少配置会阻止服务启动，不存在本地回落。
...
1. **禁默认回落**：任何服务/worker 实例启动前必须加载本 `.env`（用 `serve.py`
   或 `run_worker.sh`）。缺少 `FILE_ASSET_S3_ENDPOINT_URL` 或
   `FILE_ASSET_DATABASE_URL` 时，应用直接停止。
2. **统一端点/中心台**：要么全员共用中心台网址（推荐），要么**每台独立跑
   worker 的机器** `.env` 都显式指向 NAS MinIO 端点 + 账号
   （账号/密钥见私密 `.env`，勿写入版本库）。独立 worker 直连 MinIO/DB，**不经过网址**，
   光给网址不够。
3. **统一 NAS 元数据**：文件进同一对象存储后，元数据也必须进入 NAS 上唯一的 PostgreSQL。
   办公室与家庭电脑运行的 worker 都必须使用同一条 NAS 连接串。
...
FILE_ASSET_DATABASE_URL=<统一的 NAS Postgres 连接串>
