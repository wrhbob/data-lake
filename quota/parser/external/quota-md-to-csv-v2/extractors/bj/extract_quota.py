...
# bj 项目编码: {章}-{定额号} (无字母前缀, 与 hu 不同)
# - 北京定额编号无册号字母前缀, 形如 1-1 / 2-21 / 4-5
#   例: 1-1 = 第 1 章 第 1 个定额; 2-21 = 第 2 章 第 21 个定额
...
# ---------- bj 单表抽取 (自闭环, 不依赖 hu extract_table) ----------
def _parse_bj_material_row(cells: list[dict], start_cols: list[int], project_cols: list[int],
                            *, grid: list[list[str]] | None = None, grid_row_idx: int | None = None
                            ) -> dict | None:
    """bj 专用材料行解析 (4 列左栏: 分类/编码/名称/单位, 无单价列).

    形态 (北京消耗量定额):
      col0=人工/材料/机械 (rowspan 跨多行)
      col1=6-12 位数字编码 (如 00010306 / 3109003301 / 9905000008)
      col2=名称 (如 综合用工一类 / 成品大城样砖 / 灰浆搅拌机200L)
      col3=单位 (如 工日 / 块 / m³ / % / 台班)
      col4..N=对应定额号 1-X 的消耗量 (N 个定额号 = N 列)

    返回 dict: name / unit / quantities / category / is_other_material / is_appendix / code
    """
    cell_positions = []
    for cell, sc in zip(cells, start_cols):
        cell_positions.append({
    ...
    # v3 修复: grid_row_idx 行 col0 被前一行 rowspan 继承时, 从 grid 读实际 col0 注入
    if (grid is not None and grid_row_idx is not None
            and cell_positions and cell_positions[0]["start_col"] != 0
            and grid_row_idx < len(grid)):
        col0_text = grid[grid_row_idx][0].strip()
    ...
    # bj 形态: 第一行非空 cell 的 start_col 应 = 0 (col0 = 分类), 但也可能 colspan
    # 把 col0 撑到 col3. 取 start_col == 0 的 cell 作为分类标签.
    col0_cell = next((c for c in cell_positions if c["start_col"] == 0), None)
    if col0_cell is None or not col0_cell["text"].strip():
        return None
    raw_classifier = col0_cell["text"].strip()

    # 分类映射 (北京定额: 人工/材料/机械)
    raw_norm = raw_classifier.replace(" ", "")
    if raw_norm in ("人工",):
        category = "工"
        ...
        # 不在 3 类中 (可能是表头行"工料机名称"被 OCR 重复 emit 之类) → 跳过
        return None

    # 找编码: start_col > 0 的第一个纯数字 cell
    code_cell = None
    for c in cell_positions:
    ...
    # 找名称: 编码之后的第一个非空 cell (可能被 colspan 跳过多个)
    name_cell = None
    for c in cell_positions:
    ...
    # 找单位: 名称之后的第一个非空 cell (如 "工日"/"块"/"m³"/"台班"/"%")
    unit_cell = next(
        (c for c in cell_positions
         if c["start_col"] > name_end_col and c["text"].strip()),
        None,
    ...
    # 找 quantity: 单位之后到行末, 按 project_cols 取每个定额号对应 cell
    quantities: dict[int, str] = {}
    for pc in project_cols:
        q_val = ""
        for c in cell_positions:
    ...
    # 比例行识别: 单位=% 必定是比例行; 或者编码是 34000011 (其他材料费) / 99460004 (其他机具费)
    is_other_material = (unit == "%" or
                         "其他材料费" in name or "其它材料费" in name)
...
def extract_table_bj(html_text: str, working_content: str, unit_fallback: str,
                     section_id: str = ""
                     ) -> tuple[list[list[str]], dict | None]:
    """bj 专用单表抽取.

    抽取策略 (与 hu/gd/cq 不同):
      - 北京消耗量定额表头 3 层: 编号行 / 项目分组行 / 工料机+单位+消耗量行
      - 数据行 4 列左栏 + N 消耗量列 (无单价列)
      - 项目特征: col0="项目" 行的 col[N] 拼起来
      - 单位: 表外 fallback (work_content 后的"单位:"行) — 每个定额号共用同一个 unit_fallback
      - 价格/验证: 恒空 (bj 没有价类信息)

    10 列 schema: 类型|编码|名称|特征|单位|消耗量|基价|验证|换算|来源
    返回与 hu extract_table 同接口 (rows, meta | None).
    """
    grid, total_cols, raw_rows, cell_cols = parse_table(html_text)
    if not grid or total_cols < 5 or not raw_rows:
        return [], None
    ...
    for r, row in enumerate(grid):
        col0 = row[0].strip()
        col2 = row[2].strip() if len(row) > 2 else ""
        # bj 表头第 3 层: col0="工料机名称" / col0="名称" / col0="分类"
        if (col0.startswith("工料机名称") or col0.startswith("工料机")
                or (col0 == "" and col2 == "名称")
                or (col0 == "分类" and col2 == "名称")):
            data_start = r + 1
            break
    ...
    # 抽 materials (从 data_start 到末尾所有行, 含 forward-fill col0 分类标签)
    materials: list[dict] = []
    for ri in range(data_start, len(raw_rows)):
        mat = _parse_bj_material_row(raw_rows[ri], cell_cols[ri], project_cols,
                                     grid=grid, grid_row_idx=ri)
        if mat is not None:
            materials.append(mat)

    # 异常检测: materials 为空 / 单位空
    for mat in materials:
        if not mat["unit"]:
            issue = {
                "section_id": section_id,
                "project_id": projects[0]["id"] if projects else "-",
                "reason": f"bj 资源行缺少单位: {mat['name']}",
    ...
    #   bj: 定行 col 2 = PID (1-1, 用户要求保留项目编号);
    #        工/料/机 行 col 2 留空 — 10 位资源编码 (如 3109003301) 在 Excel 里被
    #        自动识别成科学计数法 (3.109E+09), 项目编码信息已在名称列体现.
    #        xlsx_writer._write_csv_to_sheet 对 col 2 强制 number_format='@'
    #        防止 Excel 把 "1-1" 自动识别成日期 "1月1日".
    ...
    for j in range(n_projects):
        ...
        # bj 实际是 工 → 料 → 机 段, _parse_bj_material_row 已按行序 emit, 这里直接遍历
        for mat in materials:
            row_type = mat["category"]
            q = mat["quantities"].get(project_cols[j], "")
            # 比例行 (其他材料费/其他机具费) 单位 % 保留 quantity=比例值
            if mat["is_other_material"] or mat["is_appendix"]:
...
def process_md_file(md_path: Path) -> tuple[list[list[str]], list[dict]]:
    ...
    # bj 路径检测: 用于切换到 extract_table_bj (北京定额没有价类信息, 自闭环抽取)
    md_path_str = str(md_path).replace("\\", "/").lower()
    is_bj_path = ("/tests/beijing/" in md_path_str
                  or "/ocr/beijing/" in md_path_str
                  or "/beijing/" in md_path_str)

    # 找到所有 table 的位置
    table_matches = list(re.finditer(r"<table[^>]*>.*?</table>", content, flags=re.S))

    # bj 专用: 北京定额编号无字母前缀, 册号用 "BJ" 占位
    #   - hu 模式: 从首张表定额编号首字母提取 (G/C/B...)
    #   - bj 模式: 编号形如 "1-1" / "2-21", 无册字母
    #   - 用 "BJ" 作占位, 段行编码会变成 "BJ.1" / "BJ.1.1" 形式 (不影响章节语义)
    volume = "BJ"
    ...
    #   bj 路径: 北京消耗量定额无价类信息, extract_table_bj 不接这些集合, 跳过预扫描
    ...
    for table_idx, m in enumerate(table_matches):
        ...
        # bj 路径: 走 extract_table_bj 自闭环 (无价类信息, 不需要 seen_pids/全局判断)
        if is_bj_path:
            rows, meta = extract_table_bj(table_html, wc_text, unit_fallback,
