...
│   ├── 河南上.xlsx / 河南下.xlsx
│   └── 重庆上.xlsx / 重庆下.xlsx
├── 人工挖土方_跨省对比.xlsx    # 74 条命中（AND 语义：挖 AND (含 土/淤泥/冻土/沟槽/基坑/槽坑)，排除 机械/挖掘机/挖孔/钻；按省 四川 12 / 广东 22 / 河南 27 / 重庆 13）
└── 踢脚线_跨省对比.xlsx       # 42 条命中（AND 语义：踢脚 AND (含 踢脚线 OR 踢脚板)；按省 四川 6 / 广东 20 / 河南 0 / 重庆 16）
```
...
### 加扩展词（细化主词，与 keyword 是 AND 关系）

```bash
python extract.py "踢脚" --any "踢脚线 踢脚板"
# 命中：name 同时含「踢脚」与（「踢脚线」或「踢脚板」） → 42 条（vs 主词单用 117）
```
...
python extract.py "挖" --any "土 淤泥 冻土 沟槽 基坑 槽坑" \
                    --exclude "机械 挖掘机 挖孔 钻"
# 命中：name 同时含「挖」与（任一「土/淤泥/冻土/沟槽/基坑/槽坑」），且不含「机械/挖掘机/挖孔/钻」 → 74 条
```
...
1. 任一 `exclude` 词出现 → 排除
2. **AND 主导**：name 必须含 `keyword`；`any_terms` 非空时还必须含至少一个 any 词
   - 补充词起「细化」作用：主词定主类，补充词缩到子类
   - 例：`keyword=挖` 单跑 1060 条；`挖 + any="土 淤泥 ..."` 缩到 74 条
   - 例：`keyword=踢脚` 单跑 117 条；`踢脚 + any="线 板"` 缩到 42 条
