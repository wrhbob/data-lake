...
def upload_and_parse(pdf_path: Path, out_dir: Path, api_url: str,
                    page_start: int = 0, page_end: int = 99999) -> Path:
    """上传 PDF + 调 /file_parse，返回 result.json 路径。

    2026-07-31 加 page_start/page_end：透传给 MinerU API 的 start_page_id/end_page_id，
    支持只 OCR 部分页（调试/省时用）。0-indexed，默认 0/99999（全本）。
    """
    ...
    data = {
        ...
        "table_enable": "true",
        "image_analysis": "true",
        "start_page_id": str(page_start),
        "end_page_id": str(page_end),
    }
...
def main():
    ...
    ap.add_argument("--page-start", type=int, default=0,
                    help="起始页码 0-indexed (default: 0)，传给 MinerU start_page_id")
    ap.add_argument("--page-end", type=int, default=99999,
                    help="结束页码 0-indexed (default: 99999)，传给 MinerU end_page_id")
    args = ap.parse_args()
    ...
    # 2. 上传 + 解析
    result_path = upload_and_parse(
        pdf_path, out_dir, args.api_url,
        page_start=args.page_start, page_end=args.page_end,
    )
    if args.page_start != 0 or args.page_end != 99999:
        print(f"📄 页码范围: page_idx [{args.page_start}..{args.page_end}]")
