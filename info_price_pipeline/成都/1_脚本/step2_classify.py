...
def classify(result_json, log_lines=None, city=None, period=None):
    ...
    n_seedling_override = 0  # 2026-08-11 增：苗木表 SKIP→MARKET 计数
    for idx, item in enumerate(content_list):
        if item.get("type") != "table":
            continue
        page = item.get("page_idx", 0)
        ...
        # 2026-08-11 增：苗木表兜底 MARKET（SKIP 章节里的 苗木表 → 改 MARKET）
        # 根因：26.4 PDF TOC 没有"苗木"章节，导致 p73-p76 苗木表被 build_page_to_section
        #       归到下一章"价格走势图"（SKIP）。苗木表其实是 MARKET 章节里的子段（紧接 9 区建材表）
        # 特征：body 含 "苗木价格说明"（恒出现于 苗木 章节前导说明）或多种 苗木 名（洒金珊瑚/三角梅/绣球/月季/棕竹/天竺葵/地被石竹 等）
        # 严判：必须同时有 苗木 keyword 1 个 + 「灌高」或「冠幅」 1 个 → 苗木 schema
        if section == "SKIP":
            body_seedling = item.get("table_body", "") or ""
            _SEEDLING_NAMES = ["洒金珊瑚", "三角梅", "绣球", "月季", "棕竹", "天竺葵", "地被石竹", "鼠尾草", "银叶合欢", "七里香", "蔷薇", "琴丝竹", "凤尾竹", "苦竹", "紫竹", "鸭脚木", "地被", "花卉"]
            _seedling_name_hit = any(n in body_seedling for n in _SEEDLING_NAMES)
            _seedling_signal_hit = ("苗木价格说明" in body_seedling) or ("灌高" in body_seedling and "冠幅" in body_seedling)
            if _seedling_name_hit and _seedling_signal_hit:
                section = "MARKET"
                n_caption_skip -= 1  # 不再算 caption skip
                n_seedling_override += 1
                print(f"[step2] 苗木 override: page {page} (table {idx}) SKIP → MARKET")
