...
from app.shangluo_cost_info import ingest_shangluo_cost_info_issue, list_shangluo_cost_info_issues


class ShangluoPdfAdapter:
    ...
    def discover(self, source, task, client) -> list[DiscoveredIssue]:
        ...
        max_items = int(policy.get("max_items_per_run") or 5)
        max_pages = int(policy.get("max_pages_per_run") or parser.get("max_pages_per_run") or 1)
        rows = list_shangluo_cost_info_issues(client, parser=parser, max_pages=max_pages, max_items=max_items)
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"SHANGLUO_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_shangluo_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:shangluo_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("SHANGLUO_PDF_PARSER_CONFIG_MISSING")
    return parser
