...
from app.jiuquan_cost_info import ingest_jiuquan_cost_info_issue, list_jiuquan_cost_info_issues


class JiuquanPdfAdapter:
    ...
    def discover(self, source, task, client) -> list[DiscoveredIssue]:
        ...
        rows = list_jiuquan_cost_info_issues(client, parser=parser, max_items=max_items)
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"JIUQUAN_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_jiuquan_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:jiuquan_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("JIUQUAN_PDF_PARSER_CONFIG_MISSING")
    return parser
