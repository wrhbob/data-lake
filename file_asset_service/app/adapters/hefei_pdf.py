...
class HefeiPdfAdapter:
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"HEFEI_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_hefei_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:hefei_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("HEFEI_PDF_PARSER_CONFIG_MISSING")
    return parser
