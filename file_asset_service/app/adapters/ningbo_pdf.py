...
class NingboPdfAdapter:
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"NINGBO_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_ningbo_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:ningbo_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("NINGBO_PDF_PARSER_CONFIG_MISSING")
    return parser
