...
class HubeiPdfAdapter:
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"HUBEI_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_hubei_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:hubei_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("HUBEI_PDF_PARSER_CONFIG_MISSING")
    return parser
