...
from app.qingyang_cost_info import ingest_qingyang_cost_info_issue, list_qingyang_cost_info_issues


class QingyangPdfAdapter:
    ...
    def discover(self, source, task, client) -> list[DiscoveredIssue]:
        ...
        rows = list_qingyang_cost_info_issues(client, parser=parser, max_items=max_items)
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"QINGYANG_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_qingyang_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:qingyang_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("QINGYANG_PDF_PARSER_CONFIG_MISSING")
    return parser
