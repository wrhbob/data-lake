...
from app.pingliang_cost_info import ingest_pingliang_cost_info_issue, list_pingliang_cost_info_issues


class PingliangPdfAdapter:
    ...
    def discover(self, source, task, client) -> list[DiscoveredIssue]:
        ...
        if is_history_backfill(task):
            rows = _list_all_pages(client, parser=parser, page_limit=history_page_limit(source, task, parser))
        else:
            rows = list_pingliang_cost_info_issues(client, parser=parser, max_items=max_items)
    ...
    def ingest(self, source, task, issue: DiscoveredIssue, storage) -> None:
        ...
        if row is None:
            raise ValueError(f"PINGLIANG_PDF_ROW_NOT_DISCOVERED: {issue.source_item_key}")
        ingest_pingliang_cost_info_issue(
            ...
            client=self._client,
            row=row,
            actor_id="cost-info-worker:pingliang_pdf",
...
def _active_parser(source) -> dict:
    ...
    if not isinstance(parser, dict):
        raise ValueError("PINGLIANG_PDF_PARSER_CONFIG_MISSING")
    return parser
...
def _list_all_pages(client, *, parser: dict, page_limit: int) -> list[dict]:
    ...
    for page in range(1, page_limit + 1):
        page_rows = list_pingliang_cost_info_issues(client, parser=parser, max_items=None, page_no=page)
        if not page_rows:
            break
        added = 0
