...
function previewUrlPaginated(file, sheet = 0, opts = {}) {
  if (!file?.file_id) return "";
  if (file.__virtual) {
...
function fetchXlsxPage(selectedFileId, append) {
  ...
  fetch(previewUrlPaginated(state.file, currentSheetIndex, { offset: state.offset, limit: state.limit }))
    .then((response) => {
...
function renderCell(item, column) {
  ...
  if (column.type === "status") return `<span class="status-badge status-${escapeHtml(item.status)}">${escapeHtml(statusLabels[item.status] || item.status)}</span>${renderParseBadge(item)}`;
...
function renderActions(item) {
  ...
  return `
    <div class="row-actions">
      ...
      ${item.domain_type === "cost_info" ? (
        item.parse_supported ? `
        <button class="icon-button" type="button" title="解析 PDF → xlsx（不弹框，仅状态列变化）" data-action="parse-archive" data-id="${escapeHtml(item.archive_id)}">
          <i data-lucide="play"></i>
        </button>
      ` : `
        <button class="icon-button" type="button" title="该档案城市暂未接入解析（成都/重庆已接入，其他城市待接入）" disabled>
          <i data-lucide="play"></i>
        </button>
      `
      ) : ""}
...
function renderViewer(item) {
  ...
  // 2026-08-13: 重置 viewerXlsxPane（隐藏 info 的 xlsx pane），避免定额/其他域预览被信息价 excel 污染
  const _panes = $("#viewerPanes");
  const _xlsxPane = $("#viewerXlsxPane");
  const _viewerCanvasEl = $("#viewerCanvas");
  if (_panes && _xlsxPane && _viewerCanvasEl) {
    _panes.classList.remove("dual-preview", "xlsx-only");
    _panes.classList.add("pdf-only");
    _xlsxPane.hidden = true;
    _xlsxPane.innerHTML = "";
    _viewerCanvasEl.hidden = false;
  }
  refreshIcons();
}
...
function handleClick(event) {
  ...
  if (action.dataset.action === "open-viewer") {
    // 2026-08-13: cost_info 预览交给 info 的 handleClick（弹"选择预览模式"窗口），main 不直接进详情页
    if (state.domain === "cost_info") return;
    loadArchiveDetail(action.dataset.id);
  }
...
/* ════════════════════════════════════════════════════════════════════════════
 * 2026-08-12 apply 自 info app.js
const domainConfigs = {
  cost_info: {
    title: "信息价档案台",
    resultUnit: "份信息价档案",
...
（data_lake0714info/.../app.js）
 *
 * 用途：信息价域前端逻辑（parse dialog / SSE 事件流 / 解析产物管理 / 多 sheet tab / preview 模式）。
 *       main 后端 apply 了 8 个 /parse* 端点，但 main 前端 quota-ui.js + app.js
 *       当前不消费这些端点；前端 UI 单独接之前，先把逻辑备齐避免重复工作。
 *
 * 重命名记录（2026-08-12）：
 *   - main 的 `function previewUrl(file, sheet, opts)` 重命名为 `previewUrlPaginated`
 *     （v0.4 sheet 分页 + offset/limit 续接，与 info 的 `previewUrl(file)` 单 sheet 版同名冲突）
 *   - main 内唯一调用 sheet 分页的位置 L1663 已改为 `previewUrlPaginated(...)`
 *   - 其他 6 处单参数 `previewUrl(file)` 调用点保留原名（apply 后由 info 版接管）
 *
 * ──────────────────────────────────────────────────────────────────────────── */
...
const state = {
  ...
  parseDialog: {
    ...
    archive: null,
    task_id: "",
    status: "idle", // idle | queued | running | succeeded | failed | cancelled
    log: [],
    form: { year: "", period: "" },
    error_code: "",
    error_message: "",
  ...
  mirrorExporting: false,
  viewerInfoOpen: false,
  viewerPreviewMode: "pdf",  // 2026-08-03 新增：pdf | xlsx | dual
  filterAdvancedOpen: false,
  zipPreview: { fileId: null, manifest: null, selectedIndex: null },
  filters: defaultFilters("cost_info"),
  coverageFilters: defaultCoverageFilters(),
...
function matchesRegion(item, filter, value, option) {
  ...
  // 2026-08-05 改：选省份 + 市 all 时, 按 metadata.publisher_scope（识别区域）
  // 排除市级 archive (publisher_scope="city"), 只保留省本级 (publisher_scope="province")
  // 或没 publisher_scope 的 archive (fallback 当省本级, 旧数据兼容)
  const publisherScope = metadata(item, "publisher_scope");
  if (publisherScope && publisherScope !== "province" && itemCode.startsWith(value)) {
    return false;  // 减去市级
  }
  if (item.region_code && String(item.region_code).startsWith(value)) return true;
  return rawText.includes(String(option?.label || value).toLowerCase());
...
// ===== PDF -> xlsx parse dialog (cost_info row action) =====

async function openParseStatusDialog(archiveId) {
  state.parseDialog = {
    open: true,
  ...
  try {
    const resp = await fetch(apiUrl(`/api/archives/${archiveId}`));
    if (!resp.ok) {
      const errBody = await resp.text();
      throw new Error(`Archive fetch failed (${resp.status}): ${errBody}`);
    }
    const archive = await resp.json();
    state.parseDialog.archive = archive;
    // Auto-fill year/period from filename (mirrors backend regex)
    const primary = (archive.files || []).find((f) => f.is_primary);
    const fileName = primary?.file_name || "";
    const m = fileName.match(/(\d{4})\s*年\s*(\d{1,2})\s*期/) || fileName.match(/(\d{4})[\-年](\d{1,2})/);
    if (m) {
      state.parseDialog.form.year = m[1];
      state.parseDialog.form.period = parseInt(m[2], 10).toString();
    }
    if (archive.parse_status === "succeeded") {
      state.parseDialog.status = "succeeded";
      state.parseDialog.final_xlsx_key = archive.final_xlsx_key || "";
      // 2026-08-05 改：normalize 条件用 final_xlsx_key（xlsx 在 MinIO 的真指针）
      // — 后端 delete_xlsx_output 保留 parse_metrics + parse_finished_at 作为历史，
      //   但 final_xlsx_key 会清成 None，所以看 final_xlsx_key 就能区分「真成功」vs「删除后」
    } else if (archive.parse_status == null && archive.final_xlsx_key && archive.parse_metrics?.row_count != null && archive.parse_metrics.row_count > 0) {
      state.parseDialog.status = "succeeded";
      state.parseDialog.final_xlsx_key = archive.final_xlsx_key || "";
    } else if (archive.parse_status === "failed") {
      state.parseDialog.status = "failed";
      state.parseDialog.error_code = archive.parse_error_code || "";
      state.parseDialog.error_message = archive.parse_error_message || "";
      state.parseDialog.form.year = state.parseDialog.form.year || "";
    } else if (archive.parse_status === "cancelled") {
      state.parseDialog.status = "cancelled";
    } else if (archive.parse_status === "running" || archive.parse_status === "queued") {
      state.parseDialog.task_id = archive.parse_task_id || "";
      state.parseDialog.status = archive.parse_status;
      openParseEventSource(state.parseDialog.task_id);
    }
  } catch (err) {
    state.parseDialog.status = "failed";
    state.parseDialog.error_code = "FETCH_FAILED";
    state.parseDialog.error_message = err.message || String(err);
  }
  renderParseStatusModal();
  // 2026-08-04 改：idle 状态若表单齐全（year/period）自动提交解析；succeeded/failed/cancelled 状态只展示状态，不在 modal 触发重解析
  // 重新解析统一走行级 ▶️ 按钮（modal 内不再有任何「再次解析」按钮）
  if (state.parseDialog.status === "idle") {
    if (state.parseDialog.form.year && state.parseDialog.form.period) {
      submitParseFromDialog();
    }
  }
  // 2026-08-04 改：用户主动点 ▶️ 必须立刻弹框有视觉反馈（之前「排队/运行中不弹」是错的）
  // - idle：自动提交，弹框打开（显示「即将解析…」短暂空白然后 queued/running）
  // - queued / running：弹框打开（实时显示解析日志 + spinner）
  // - succeeded / failed / cancelled：弹框打开（成功有下载按钮，失败有重试按钮）
  state.parseDialog.open = true;
  renderParseStatusModal();
  // 2026-08-04 改：banner 已 100% 走 dialog.archive.parse_metrics（后端权威值，统一架构）
  // 之前 v5 引入 DOMParser 兜底 + _rowCountCache，但会产生「后端 1923 但前端 495」缓存污染 bug
  // 删除 fetchRowCountFromPreview 触发逻辑 — 后端若没 row_count 字段，banner 显示 loading + 灰色「—」
  // 由用户主动刷新页面让后端补算（或重新解析一次）
}

function closeParseStatusDialog() {
  const dialog = state.parseDialog;
  // 2026-08-04 改：关闭 modal 不停 SSE — 否则行级 status 列 badge 永远拿不到终态事件
  // SSE 订阅由 handleParseEvent 在 terminal 状态时自动关闭（eventsource 自我清理）
  // 或下次 openParseStatusDialog 时旧 dialog.es 才会被替换
  dialog.open = false;
  renderParseStatusModal();
  // 2026-08-04 改：关闭 modal 兜底 — 强制刷一次 list 让行级 badge 立刻反映最新状态
  ...
  if (dialog.status === "running" || dialog.status === "queued") {
    startParseBadgePolling();
  }
}

async function submitParseFromDialog(event) {
  if (event) event.preventDefault();
  await _runParseSubmit({ showModal: true });
}

// 2026-08-05 新增：行级 ▶️ 按钮直接提交解析，不弹任何框
// 用户反馈：「点 ▶️ 不要弹框，只状态列变化」
async function submitParseInBackground(archiveId) {
  // 同步准备 dialog 上下文（form.year/period 从文件名正则抽），但不打开 modal
  state.parseDialog = {
    open: false,
    archive_id: archiveId,
    archive: null,
  ...
  try {
    ...
    // 抽 year/period 失败也不阻断——后端会从 filename 抽
  }
  await _runParseSubmit({ showModal: false });
}

async function _runParseSubmit({ showModal }) {
  const dialog = state.parseDialog;
  // 2026-07-31 改：删 INVALID_INPUT 校验（用户原话：PDF 名固定不需要填年份期数，正则自动抽）
  const year = parseInt(dialog.form.year, 10) || 0;
  const period = parseInt(dialog.form.period, 10) || 0;
  dialog.status = "queued";
  dialog.error_code = "";
  dialog.error_message = "";
  dialog.log = [];
  dialog.runningStartedAt = Date.now();  // 2026-08-04 新增：记开始时间，2s 锁「解析中」
  // 2026-08-05 改：showModal=false 时不打开弹框（点 ▶️ 后只状态列变化）
  ...
  try {
    const resp = await fetch(apiUrl(`/api/archives/${dialog.archive_id}/parse`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ year, period: period.toString() }),
    });
    if (!resp.ok) {
      const errBody = await resp.text();
      throw new Error(`Submit failed (${resp.status}): ${errBody}`);
    }
    ...
    dialog.error_code = "SUBMIT_FAILED";
    dialog.error_message = err.message || String(err);
    if (row) {
      row.parse_status = "failed";
      // submit 失败不算 running，terminal 立即应用
      row._runningStartedAt = null;
    }
    stopParseBadgePolling();
    if (showModal) renderParseStatusModal();
    else showToast(`解析提交失败：${err.message || err}`, "error");
  }
}

function openParseEventSource(taskId) {
  const dialog = state.parseDialog;
  if (dialog.es) dialog.es.close();
  const es = new EventSource(apiUrl(`/api/archives/${dialog.archive_id}/parse-stream`));
  dialog.es = es;
  es.onmessage = (evt) => handleParseEvent(evt);
  es.onerror = () => {
    if (dialog.status === "running" || dialog.status === "queued") {
      dialog.status = "failed";
      dialog.error_code = "SSE_DISCONNECTED";
      dialog.error_message = "日志连接断开，请重试。";
    }
    es.close();
    dialog.es = null;
    renderParseStatusModal();
...
function handleParseEvent(evt) {
  ...
  if (payload.truncated) {
    ...
    dialog.dropped_lines = payload.dropped_lines || dialog.dropped_lines;
  }
  if (payload.line) dialog.log.push(payload.line);
  if (payload.status) dialog.status = payload.status;
  if (payload.final_xlsx_key) dialog.final_xlsx_key = payload.final_xlsx_key;
  if (payload.parse_error_code) dialog.error_code = payload.parse_error_code;
  if (payload.parse_error_message) dialog.error_message = payload.parse_error_message;
  // 2026-08-05 修：同步 row.parse_status 到 state.archives + renderRows() 让表格里状态列 badge 实时刷新
  ...
  if (["succeeded", "failed", "cancelled"].includes(payload.status)) {
    ...
    // 2026-08-04 改：至少 2 秒「解析中」后再切 terminal（即使 cache-hit 1 秒就跑完）
    _scheduleTerminalTransition(dialog, payload);
  }
  renderParseStatusModal();
}
...
async function cancelActiveParse() {
  const dialog = state.parseDialog;
  if (!dialog.archive_id) return;
  // 2026-08-04 改：立刻乐观更新 status = "cancelling"，让用户看到反馈（避免「以为没反应再点一次」）
  // 后端真正 cancel 完才发 SSE cancelled 终态，那时再切 "cancelled"
  dialog.status = "cancelling";
  dialog.log = dialog.log || [];
  dialog.log.push("[cancelled by user]");
  renderParseStatusModal();
  try {
    await fetch(apiUrl(`/api/archives/${dialog.archive_id}/parse-cancel`), { method: "POST" });
  } catch (err) {
    console.warn("cancel request failed", err);
    dialog.status = "running";  // 2026-08-04 改：cancel 失败回退到 running
    renderParseStatusModal();
  }
}

// 2026-08-03 新增：行级下载 Excel 按钮走 inline onclick（绕过事件委托链 + Lucide SVG 替换）
window.__dlXlsx = async function downloadParseOutputInline(archiveId) {
  try {
    const fileResp = await fetch(apiUrl(`/api/archives/${archiveId}/parse-output`));
    if (!fileResp.ok) throw new Error(`File fetch failed (${fileResp.status})`);
    const blob = await fileResp.blob();
    const disp = fileResp.headers.get("Content-Disposition") || "";
    const m = disp.match(/filename="?([^";]+)"?/);
    const filename = m ? m[1] : "parse-output.xlsx";
...
async function downloadParseOutput() {
  const dialog = state.parseDialog;
  if (!dialog.archive_id) return;
  try {
    // 2026-08-03 改：后端直接代理 MinIO xlsx 字节，单次 fetch 同源 → blob → a.click()（零跨域，Content-Disposition 头带文件名）
    const fileResp = await fetch(apiUrl(`/api/archives/${dialog.archive_id}/parse-output`));
    if (!fileResp.ok) throw new Error(`File fetch failed (${fileResp.status})`);
    const blob = await fileResp.blob();
    const disp = fileResp.headers.get("Content-Disposition") || "";
    const m = disp.match(/filename="?([^";]+)"?/);
    ...
    dialog.error_code = "DOWNLOAD_FAILED";
    dialog.error_message = err.message || String(err);
    renderParseStatusModal();
  }
}

async function deleteParseOutput() {
  const dialog = state.parseDialog;
  if (!dialog.archive_id) return;
  if (!confirm("确定删除已解析的 Excel 吗？文件将从 NAS 删除，档案元数据保留历史。")) return;
  try {
    const resp = await fetch(apiUrl(`/api/archives/${dialog.archive_id}/parse-output`), {
      method: "DELETE",
    });
    if (!resp.ok) {
      const errBody = await resp.text();
...
function renderParseStatusModal() {
  const modal = $("#parseStatusModal");
  if (!modal) return;
  const dialog = state.parseDialog;
  modal.hidden = !dialog.open;
  modal.setAttribute("aria-hidden", String(!dialog.open));
  ...
  // 2026-08-04 用户原话：「解析和再次解析的状态不要放在弹窗里面，放状态栏内里 已完成内里」
  // 状态徽章（解析中 / 已完成 / 失败 / 取消）只在行级状态列渲染（renderParseBadge），
  // modal 头部不重复显示状态徽章 — modal 只承担动作（取消/下载/删除）和日志/错误详情。
  // 这里只放一行简短导引，告诉用户「去行上看状态」即可。
  const statusHint = (() => {
    const map = {
      idle:      { text: "即将解析…",                                                      icon: "circle-dashed" },
      queued:    { text: "已排队，状态会在「状态」列实时刷新",                  icon: "loader" },
      running:   { text: "正在解析，状态会在「状态」列实时刷新",                icon: "loader" },
      cancelling:{ text: "正在取消…",                                          icon: "loader" },
      succeeded: { text: "解析已完成，状态徽章已固化在「状态」列",              icon: "check-circle-2" },
      failed:    { text: "解析失败，详情见下方错误信息",                        icon: "alert-circle" },
      cancelled: { text: "已取消，可重新发起解析",                              icon: "x-circle" },
    };
    const entry = map[dialog.status] || { text: "状态未知", icon: "circle" };
    return `
      <div class="parse-status-hint">
        <i data-lucide="${entry.icon}"></i>
        <span>${escapeHtml(entry.text)}</span>
      </div>`;
  })();
  // 2026-08-04 改：彻底删除「开始解析」兜底按钮
  // 用户原话：「我只要点击解析按钮 开始解析就行了」—— 直接通过行级 ▶️ 触发，自动提交
  // modal 内不再需要任何「开始解析」手动按钮
  const startButtonHtml = "";
  const isInFlight = dialog.status === "running" || dialog.status === "queued" || dialog.status === "idle";
  // 2026-08-04 改：用户要求不显示解析日志（避免 Python SyntaxWarning / 路径乱码 / 中间步骤噪音刷屏）
  // 日志仍在 dialog.log 里收集（前端调试可用 console.log 查看），但 modal 主体不渲染
  const logHtml = "";
  const errorHtml = dialog.error_message ? `
    <div class="parse-error">
      <div class="parse-error-header">
        <i data-lucide="alert-triangle"></i>
        <strong>错误码：${escapeHtml(dialog.error_code || "UNKNOWN")}</strong>
      </div>
      <pre>${escapeHtml(dialog.error_message)}</pre>
    </div>
  ` : "";
  // 2026-08-04 改：banner 永远只读 dialog.archive.parse_metrics（后端权威数字）
  // 之前 v5 引入 _rowCountCache 兜底（DOMParser 数 tr）导致「后端 1923 但前端 495」的缓存污染 bug
  // 既然 _serialize_archive_base 已经序列化 parse_metrics.{row_count,xlsx_size,xlsx_sha256,duration_seconds}，
  // modal banner / xlsxInfo 都不再需要 DOMParser 兜底。
  // 唯一保留 loading 态：dialog.archive 还没 fetch 进来（< 50ms 窗口）。
  const rowCountBanner = (() => {
    if (dialog.status !== "succeeded") return "";
    const m = dialog.archive?.parse_metrics || null;
    const rowCount = m?.row_count;
    if (typeof rowCount === "number" && rowCount > 0) {
      return `
        <div class="parse-row-count">
          <i data-lucide="bar-chart-3"></i>
          <span class="parse-row-count-num">${rowCount.toLocaleString("zh-CN")}</span>
          <span class="parse-row-count-unit">条</span>
        </div>`;
    }
    // archive 还没 fetch 进来 — loading 态（< 50ms 窗口，UI 几乎看不到）
    return `
      <div class="parse-row-count parse-row-count-loading" data-archive-id="${escapeHtml(dialog.archive_id)}">
        <i data-lucide="loader"></i>
        <span class="parse-row-count-num">…</span>
        <span class="parse-row-count-unit">条</span>
      </div>`;
  })();
  const xlsxInfo = (() => {
    if (dialog.status !== "succeeded") return "";
    // 2026-08-04 修：m 从 dialog.archive 取（同上）
    const m = dialog.archive?.parse_metrics || null;
    const sizeKb = m?.xlsx_size ? (m.xlsx_size / 1024).toFixed(1) + " KB" : "—";
    const dur = m?.duration_seconds != null ? m.duration_seconds.toFixed(1) + " 秒" : "—";
    const sha = m?.xlsx_sha256 ? m.xlsx_sha256.slice(0, 16) + "…" : "—";
    return `
      <div class="parse-result-meta">
        <div class="parse-result-meta-item">
        ...
        <div class="parse-result-meta-item parse-result-meta-sha" title="${escapeHtml(m?.xlsx_sha256 || "")}">
          <i data-lucide="hash"></i>
          <span>sha256:${escapeHtml(sha)}</span>
        </div>
      </div>`;
  })();
  const actionButtons = (() => {
    const buttons = [];
    if (dialog.status === "running" || dialog.status === "queued") {
      buttons.push(`<button class="parse-btn parse-btn-ghost" type="button" data-action="cancel-parse">
        <i data-lucide="x"></i><span>取消解析</span>
      </button>`);
    }
    // 2026-08-04 改：cancelling 状态显示「正在取消…」+ 禁用按钮，避免用户「以为没反应再点一次」
    // 不显示任何按钮（避免重复触发）
    if (dialog.status === "succeeded") {
      // 2026-08-04 改：彻底删除「再次解析」按钮
      // 用户原话：「我只要点击解析按钮 开始解析就行了」—— 重新解析要走行级 ▶️，
      // modal 内只保留「下载 Excel」+「删除 Excel」
      buttons.push(`<button class="parse-btn parse-btn-primary" type="button" data-action="download-parse-output">
        <i data-lucide="download"></i><span>下载 Excel 文件</span>
      </button>`);
      buttons.push(`<button class="parse-btn parse-btn-danger-ghost" type="button" data-action="delete-parse-output">
        <i data-lucide="trash-2"></i><span>删除 Excel 文件</span>
      </button>`);
    }
    if (dialog.status === "failed") {
      // 2026-08-04 改：彻底删除「再次解析」按钮（同上，失败也要走行级 ▶️）
      buttons.push(`<button class="parse-btn parse-btn-danger-ghost" type="button" data-action="delete-parse-output">
        <i data-lucide="trash-2"></i><span>删除 Excel 文件</span>
      </button>`);
    }
  ...
  modal.innerHTML = `
    <section class="parse-dialog" role="dialog" aria-modal="true" aria-labelledby="parseStatusTitle">
      <header class="parse-dialog-header">
        <div class="parse-dialog-titles">
          <p class="eyebrow">PDF → xlsx 解析</p>
          <h2 id="parseStatusTitle">${escapeHtml(archive.title || dialog.archive_id)}</h2>
        </div>
        <button class="icon-button" type="button" title="关闭" data-action="close-parse-status">
          <i data-lucide="x"></i>
        </button>
      </header>
      ...
      <footer class="parse-dialog-footer">
        ${actionButtons}
      </footer>
    </section>
  `;
  if (typeof refreshIcons === "function") refreshIcons();
  // Wire form input changes
  modal.querySelectorAll("[data-parse-field]").forEach((el) => {
    el.addEventListener("input", (e) => {
      const field = e.target.dataset.parseField;
      dialog.form[field] = e.target.value;
    });
  });
  // Auto-scroll log to bottom
  const logEl = modal.querySelector(".parse-log");
...
async function loadArchiveDetail(id) {
  ...
  try {
    ...
    state.selectedFileId = defaultViewerFile(state.selectedArchive)?.file_id || null;
    state.viewerInfoOpen = false;
    // 2026-08-03：点眼睛后先弹「预览模式」选择（PDF / Excel / 双预览），选完才进 viewer
    openPreviewModeModal(state.selectedArchive);
  } catch (error) {
    state.error = error.message || "档案详情加载失败";
    renderApiState();
  } finally {
...
function openPreviewModeModal(archive) {
  const modal = $("#filePickerModal");
  if (!modal) return;
  $("#filePickerTitle").textContent = `选择预览模式 · ${archive.title || ""}`;
  renderPreviewModeBody(archive);
  modal.hidden = false;
...
function renderPreviewModeBody(archive) {
  const body = $("#filePickerBody");
  if (!body) return;
  const parseStatus = archive.parse_status;
  const hasFinalXlsx = !!archive.final_xlsx_key;
  const xlsxDisabled = parseStatus !== "succeeded" || !hasFinalXlsx;
  const items = [
    { mode: "pdf", icon: "file-text", label: "PDF（仅 PDF 原件）", desc: "全屏查看 PDF 原件" },
    { mode: "xlsx", icon: "file-spreadsheet", label: "Excel（仅解析后的 Excel）", desc: xlsxDisabled ? "该档案尚未生成 Excel（final_xlsx_key 为空）" : "全屏查看解析后的 Excel 表格", disabled: xlsxDisabled },
    { mode: "dual", icon: "columns-2", label: "双预览（左 PDF 右 Excel）", desc: xlsxDisabled ? "需先生成 Excel 才能双预览" : "左半 PDF + 右半 Excel", disabled: xlsxDisabled },
  ];
  body.innerHTML = items.map((it) => `
    <button type="button" class="file-picker-item" data-action="pick-preview-mode" data-mode="${it.mode}" ${it.disabled ? "disabled" : ""}>
      <span class="file-avatar type-${it.mode === 'xlsx' ? 'excel' : (it.mode === 'pdf' ? 'pdf' : 'pdf')}"><i data-lucide="${it.icon}"></i></span>
      <span class="file-picker-item-text">
        <strong>${escapeHtml(it.label)}</strong>
        <small>${escapeHtml(it.desc)}</small>
      </span>
      <i data-lucide="chevron-right" class="file-picker-item-arrow"></i>
    </button>
  `).join("");
}
...
async function handleWithdrawArchive(id) {
  ...
  const confirmed = window.confirm
    ? window.confirm(`确认删除“${archive.title}”？\n\n档案将从列表撤下（不会删除 NAS PDF 原件），同时一并删除已解析的 Excel 文件。`)
    : true;
  if (!confirmed) return;
  state.loading = true;
  renderApiState();
  try {
    // 2026-08-05 改：删 PDF 档案时一并删除已生成的 Excel（避免孤儿 xlsx）
    if (archive.final_xlsx_key || archive.parse_status === "succeeded") {
      try {
        const xlsxResp = await fetch(apiUrl(`/api/archives/${archive.archive_id}/parse-output`), { method: "DELETE" });
        if (!xlsxResp.ok && xlsxResp.status !== 404) {
          const errBody = await xlsxResp.text();
          throw new Error(`Excel 清理失败 (${xlsxResp.status}): ${errBody}`);
        }
      } catch (xlsxErr) {
        showToast(`Excel 清理失败但继续撤下档案：${xlsxErr.message || xlsxErr}`, "error");
      }
    }
    await withdrawArchive(archive);
...
// 2026-08-05 新增：仅删除 Excel（不动 PDF / archive）
async function handleDeleteArchiveXlsx(id) {
  // 2026-08-05 改：删除前先 fetch 最新 archive（state.archives 可能是 stale 的）
  // 否则误判「没有可删除的 Excel」/ 或删除后状态栏仍显示「已完成」
  let archive = archiveById(id);
  if (!archive) {
    showToast("未找到档案。", "error");
    return;
  }
  try {
    const resp = await fetch(apiUrl(`/api/archives/${archive.archive_id}`));
    if (resp.ok) {
      ...
      if (target) Object.assign(target, {
        ...
        final_xlsx_key: fresh.final_xlsx_key,
        parse_finished_at: fresh.parse_finished_at,
      });
      archive = fresh;
    }
  } catch (e) {
  ...
  if (!archive.final_xlsx_key) {
    // 区分两种情况：「从未解析过」vs「已删除」
    const wasParsed = archive.parse_finished_at || archive.parse_metrics;
    showToast(
      wasParsed
        ? "Excel 已删除（无需重复操作）。"
        : "该档案尚未解析，没有可删除的 Excel 文件。",
      "error"
    );
    return;
  }
  const confirmed = window.confirm
    ? window.confirm(`确认仅删除「${archive.title}」已解析的 Excel 文件？\n\nPDF 档案和列表行不会被删除。`)
    : true;
  if (!confirmed) return;
  state.loading = true;
  renderApiState();
  try {
    const resp = await fetch(apiUrl(`/api/archives/${archive.archive_id}/parse-output`), { method: "DELETE" });
    if (!resp.ok) {
      const errBody = await resp.text();
    ...
    state.loading = false;
    state.error = error.message || "Excel 删除失败";
    showToast(`删除失败：${state.error}`, "error");
    renderApiState();
  }
}
...
function renderCell(item, column) {
  ...
  if (column.type === "channel") return `<span class="channel-pill">${escapeHtml(channelLabels[item.channel_type] || item.channel_type)}</span>`;
  if (column.type === "attachments") return renderAttachmentCount(item);
  // 2026-07-31 改：解析状态徽章（已完成/解析中等）从操作列挪到状态列，避免操作列太挤
  if (column.type === "status") return `<span class="status-badge status-${escapeHtml(item.status)}">${escapeHtml(statusLabels[item.status] || item.status)}</span>${renderParseBadge(item)}`;
  if (column.type === "actions") return renderActions(item);
  return escapeHtml(item[column.key] || "-");
}
...
// ============================================================================
// Row-level parse badge (Phase 5R)
// ============================================================================
const PARSE_BADGE_MAP = {
  queued:    { label: "排队",   cls: "info",    icon: "loader" },
  running:   { label: "解析中", cls: "warning", icon: "loader" },
  succeeded: { label: "已完成", cls: "success", icon: "check-circle-2" },
  failed:    { label: "失败",   cls: "error",   icon: "alert-circle" },
  cancelled: { label: "已取消", cls: "warning", icon: "x-circle" },
};

function renderParseBadge(item) {
  // 2026-08-05 改：normalize 条件用 final_xlsx_key（xlsx 在 MinIO 的真指针）
  // — 后端 delete_xlsx_output 保留 parse_metrics + parse_finished_at 作为历史，
  //   但 final_xlsx_key 会清成 None，所以看 final_xlsx_key 就能区分「真成功」vs「删除后」
  // — 旧 archive (parse_status=NULL + final_xlsx_key 还有值 + parse_metrics.row_count>0) 视为 succeeded
  let status = item.parse_status;
  if (!status && item.final_xlsx_key && item.parse_metrics?.row_count != null && item.parse_metrics.row_count > 0) {
    status = "succeeded";
  }
  if (!status || !PARSE_BADGE_MAP[status]) return "";
  const m = PARSE_BADGE_MAP[status];
  const spinning = status === "running" || status === "queued";
  return `<button class="parse-badge ${m.cls}${spinning ? " spinning" : ""}" type="button"
                  title="点击查看解析详情"
                  data-action="open-parse-from-badge" data-id="${escapeHtml(item.archive_id)}">
            <i data-lucide="${m.icon}"></i>
            <span>${m.label}</span>
          </button>`;
}

let _parseBadgePollTimer = null;
// 2026-08-04 新增：「解析中」badge 至少显示 2 秒，即使后端 cache-hit 1 秒就跑完
// — 用户反馈"第一次点解析没看到解析中状态"，原因是 parse 太快 badge 直接跳到「已完成」
// — 用 runningStartedAt 时间戳延后 terminal transition
const PARSE_BADGE_MIN_RUNNING_MS = 2000;
let _parseTerminalTimer = null;

function _scheduleTerminalTransition(dialog, payload) {
  if (_parseTerminalTimer) {
    clearTimeout(_parseTerminalTimer);
    _parseTerminalTimer = null;
  }
  const startedAt = dialog.runningStartedAt;
  if (!startedAt) {
    _applyTerminalTransition(dialog, payload);
    return;
  }
  const elapsed = Date.now() - startedAt;
  if (elapsed >= PARSE_BADGE_MIN_RUNNING_MS) {
    _applyTerminalTransition(dialog, payload);
    return;
  }
  _parseTerminalTimer = setTimeout(() => {
    _parseTerminalTimer = null;
    _applyTerminalTransition(dialog, payload);
  }, PARSE_BADGE_MIN_RUNNING_MS - elapsed);
}
...
function _applyTerminalTransition(dialog, payload) {
  ...
  if (row) {
    ...
    if (payload.parse_phase) row.parse_phase = payload.parse_phase;
    if (payload.parse_finished_at) row.parse_finished_at = payload.parse_finished_at;
    if (payload.parse_error_code !== undefined) row.parse_error_code = payload.parse_error_code;
    // 2026-08-04 新增：row_count / duration_seconds 从 SSE payload 同步到 row.parse_metrics
    // 这样 modal 重渲染时 xlsxInfo / row-count banner 能读到数字（modal 期间 row 不一定被刷过）
    if (payload.row_count != null || payload.duration_seconds != null) {
      row.parse_metrics = {
        ...(row.parse_metrics || {}),
        ...(payload.row_count != null ? { row_count: payload.row_count } : {}),
  ...
  // failed/cancelled 不弹（避免误读为成功）
  if (payload.status === "succeeded") {
    const rowCount = payload.row_count != null
      ? Number(payload.row_count).toLocaleString("zh-CN")
      : null;
    const dur = payload.duration_seconds != null
      ? formatDuration(payload.duration_seconds)
      : null;
    if (rowCount != null && dur) {
      showParseSuccessToast(rowCount, dur, dialog.archive_id);
    }
  }
}

// 2026-08-04 新增：右下角 toast「已解析 N 条 · 用时 X 秒」（3 秒自动消失）
function showParseSuccessToast(rowCount, durationLabel, archiveId) {
  // 复用 #manualUploadToast 还是新建？新建以免污染上传 toast 队列
  let toast = document.getElementById("parseSuccessToast");
  if (!toast) {
    toast = document.createElement("div");
    toast.id = "parseSuccessToast";
    toast.className = "toast parse-success-toast";
  ...
  toast.innerHTML = `
    ...
    <span><strong>已解析 ${escapeHtml(rowCount)} 条</strong> · 用时 ${escapeHtml(durationLabel)}</span>
  `;
  toast.hidden = false;
  toast.classList.remove("fade-out");
  // force reflow 让淡入动画重启
  ...
  toast._hideTimer = setTimeout(() => {
    ...
    setTimeout(() => { toast.hidden = true; toast.classList.remove("show"); }, 350);
  }, 3000);
}
...
function startParseBadgePolling() {
  ...
  const tick = async () => {
    ...
    for (const row of active) {
      try {
        const data = await requestJson(`/api/archives/${encodeURIComponent(row.archive_id)}/parse-status`);
        // 同样延后 terminal 切换（处理别处触发的 parse）
        if (["succeeded", "failed", "cancelled"].includes(data.parse_status)) {
          // 2026-08-04 改：terminal 数据挂在 row 上暂存，不直接改 row.parse_status；
          // 真正切状态由 _applyTerminalTransition 统一处理（保证 ≥2s「解析中」）
          row._pendingTerminal = {
            parse_status: data.parse_status,
            parse_phase: data.parse_phase,
            parse_error_code: data.parse_error_code,
            parse_finished_at: data.parse_finished_at,
          };
          const syntheticDialog = {
            archive_id: row.archive_id,
            error_code: data.parse_error_code || "",
            runningStartedAt: row._runningStartedAt || null,
          };
          // 第一次发现 terminal 时启动延时；后续 poll 不要再 schedule（已经在 timer 里）
          if (row._runningStartedAt !== null) {
            _scheduleTerminalTransition(syntheticDialog, row._pendingTerminal);
            row._runningStartedAt = null;  // 标记已 schedule
          }
        } else {
          row.parse_status = data.parse_status;
          row.parse_phase = data.parse_phase;
          row.parse_error_code = data.parse_error_code;
          row.parse_finished_at = data.parse_finished_at;
        }
        // Refresh just the badge cell for this row to avoid full re-render flicker.
        const btn = document.querySelector(`[data-action="open-parse-from-badge"][data-id="${CSS.escape(row.archive_id)}"]`);
        if (btn) {
          const fresh = renderParseBadge(row);
          if (fresh) { btn.outerHTML = fresh; }
          else { btn.remove(); }
          if (window.lucide) window.lucide.createIcons();
        }
      } catch (e) {
  ...
  _parseBadgePollTimer = setInterval(tick, 10000); // 10s per user spec
  tick();
}

function stopParseBadgePolling() {
  if (_parseBadgePollTimer) { clearInterval(_parseBadgePollTimer); _parseBadgePollTimer = null; }
}

// 2026-08-05 历史 register 脚本入湖的 archive（成都 + 内蒙古 12 个城市）会被打 manual_upload 标签，
// 但前端用 channel_type === "manual_upload" 判断「编辑元数据 / 删除整档」按钮就不对——
// 这些 archive 是脚本批量入的，不是用户网页手动上传，不该走「编辑/整删」流程，要走「撤档 / 删 Excel」流程。
// channel_type / collection_method / source_id 都无法可靠区分（脚本和网页 modal 都用同一组合），
// 唯一可靠的区分是 region_code 黑名单。
function isRegisterScriptArchive(item) {
  const rc = item.region_code;
  if (!rc) return false;
  if (rc === "510100") return true;  // 成都 register 脚本
  if (rc >= "150000" && rc < "153000") return true;  // 内蒙古 12 个 register 城市
  return false;
}
...
function renderActions(item) {
  ...
  const href = downloadUrl(file);
  const isRegisterScript = isRegisterScriptArchive(item);
  // 2026-08-05 改：「编辑元数据 / 删除整档」按钮只对真网页手动上传的 archive 显示，
  // register 脚本入湖的 archive 走 details 菜单（PDF 撤档 + Excel 删除）
  const manualActions =
    item.channel_type === "manual_upload" && !isRegisterScript
      ? `
        <button class="icon-button" type="button" title="编辑元数据" data-action="edit-archive" data-id="${escapeHtml(item.archive_id)}">
  ...
  const withdrawAction =
    item.channel_type !== "manual_upload" || isRegisterScript
      ? `
        <details class="download-menu">
          <summary class="icon-button danger-action" title="删除选项（PDF 档案 / Excel 文件）" data-action="delete-menu-toggle">
            <i data-lucide="trash-2"></i>
          </summary>
          <div class="download-menu-list">
            <button class="icon-button danger-action" type="button" title="删除 PDF 档案（撤下整档，Excel 一并删除）" data-action="withdraw-archive" data-id="${escapeHtml(item.archive_id)}">
      ...
      `
      : "";
  // E2: 2026-08-03 下载按钮改 details 折叠菜单，PDF + xlsx 两选项
  // 按 R8 解释：details = HTML 原生可折叠菜单，点 summary 展开/收起，无需 JS
  const download = `
    <details class="download-menu">
      <summary class="icon-button" title="下载选项">
        <i data-lucide="download"></i>
      </summary>
      <div class="download-menu-body">
        ${href
          ...
          `
          : `
            <span class="download-menu-item disabled" title="暂无原件">
              <i data-lucide="file-text"></i> 暂无原件
            </span>
          `
        }
...
function renderViewerShell(item) {
  ...
  const summary = mirrorSummary(files);
  const disabled = state.mirrorExporting ? "disabled" : "";
  // 2026-08-10 补：viewer 顶部 3 个 mode 切换按钮（PDF / xlsx / 双 pane）。
  // zip 副本 (8-07 11:43) 写了容器/样式/状态机，唯独漏了顶部按钮。
  // handleClick L5703 已处理 preview-mode-pdf / -xlsx / -dual 三个 action。
  const mode = state.viewerPreviewMode;
  const fileId = encodeURIComponent(item.archive_id);
  const modeBtn = (target, icon, label) => `
    <button class="tool-button ${mode === target ? "active" : ""}" type="button"
            data-action="preview-mode-${target}" data-id="${fileId}"
            title="切换到 ${label} 视图">
  ...
  $("#viewerTools").innerHTML = `
    ${files.length > 1 ? `<span>${files.length} 个附件</span>` : ""}
    ${summary ? `<span class="viewer-mirror-summary">${escapeHtml(summary)}</span>` : ""}
    ${item.domain_type === "cost_info" && item.parse_supported ? `
      <div class="viewer-mode-group" role="group" aria-label="预览模式">
        ${modeBtn("pdf", "file-text", "PDF")}
        ${modeBtn("xlsx", "table-2", "xlsx")}
        ${modeBtn("dual", "columns-2", "双 pane")}
      </div>
    ` : ""}
    <button class="tool-button" type="button" data-action="mirror-archive" ${disabled}>
      <i data-lucide="folder-sync"></i>
...
function renderViewer(item) {
  ...
  renderAttachmentList(item);
  renderViewerCanvas(item, file);
  // 2026-08-03 新增：根据 viewerPreviewMode 切 viewerPanes 样式 + 显隐 xlsx pane
  const panes = $("#viewerPanes");
  const xlsxPane = $("#viewerXlsxPane");
  const viewerCanvasEl = $("#viewerCanvas");
  if (panes && xlsxPane && viewerCanvasEl) {
    const mode = state.viewerPreviewMode;
    panes.classList.remove("dual-preview", "xlsx-only", "pdf-only");
    if (mode === "dual") {
      // 双 pane：左右各一半，左 PDF 右 xlsx
      panes.classList.add("dual-preview");
      xlsxPane.hidden = false;
      viewerCanvasEl.hidden = false;
    } else if (mode === "xlsx") {
      // 全屏 xlsx：viewerCanvas 占位隐藏，xlsxPane 独享全宽
      panes.classList.add("xlsx-only");
      xlsxPane.hidden = false;
      viewerCanvasEl.hidden = true;
      viewerCanvasEl.innerHTML = "";
    } else {
      // 默认全屏 PDF：xlsxPane 隐藏
      panes.classList.add("pdf-only");
      xlsxPane.hidden = true;
      xlsxPane.innerHTML = "";
      viewerCanvasEl.hidden = false;
    }
  }
...
// 2026-08-03 新增：拉取并注入 xlsx HTML 预览到 #viewerXlsxPane（双 pane / Excel 模式用）
async function loadXlsxPreviewPane(archiveId) {
  const pane = $("#viewerXlsxPane");
  if (!pane) return;
  if (!archiveId) { pane.hidden = true; pane.innerHTML = ""; return; }
  pane.hidden = false;
  pane.innerHTML = `<div class="empty-state"><strong>正在加载 Excel 预览…</strong></div>`;
  try {
    const resp = await fetch(apiUrl(`/api/archives/${archiveId}/parse-output-full-preview`));
    if (!resp.ok) {
      const hint = resp.status === 404
        ? "该档案尚未生成 Excel（final_xlsx_key 为空）。请回到列表页点击行级 ▶️ 触发解析。"
        : `该档案可能尚未解析完成（${resp.status}）。`;
      pane.innerHTML = `<div class="empty-state"><strong>Excel 暂不可用</strong><span>${hint}</span></div>`;
      return;
    }
    pane.innerHTML = await resp.text();
  } catch (err) {
    pane.innerHTML = `<div class="empty-state"><strong>网络错误</strong><span>${escapeHtml(err.message || String(err))}</span></div>`;
  }
}

// 2026-08-03 新增：拉取完整版 xlsx HTML 替换当前 #viewerXlsxPane 内容（不下载）
async function loadXlsxFullPreviewPane(fullUrl) {
  const pane = $("#viewerXlsxPane");
  if (!pane || !fullUrl) return;
  pane.innerHTML = `<div class="empty-state"><strong>正在加载完整 Excel…</strong></div>`;
  try {
    const resp = await fetch(fullUrl);
    if (!resp.ok) {
      pane.innerHTML = `<div class="empty-state"><strong>完整 Excel 加载失败 (${resp.status})</strong></div>`;
      return;
    }
    pane.innerHTML = await resp.text();
  } catch (err) {
...
// 2026-08-04 新增：切换 Excel 工作表显示（基于 data-action="show-sheet" 按钮）
function switchExcelSheet(trigger) {
  const root = trigger.closest(".excel-sheet-tabs");
  if (!root) return;
  const targetIndex = String(trigger.dataset.sheetIndex || "");
  if (!targetIndex) return;
  // 更新 tab 状态
  root.querySelectorAll(".excel-sheet-tab").forEach((tab) => {
    const isActive = tab.dataset.sheetIndex === targetIndex;
    tab.classList.toggle("active", isActive);
    tab.setAttribute("aria-pressed", isActive ? "true" : "false");
  });
  // 切换 pane 可见性（pane 是 tabs 的兄弟节点，在父容器内查找）
  const parent = root.parentElement;
  if (!parent) return;
  parent.querySelectorAll(".excel-sheet-pane").forEach((pane) => {
    const isActive = pane.dataset.sheetIndex === targetIndex;
    pane.hidden = !isActive;
  });
  // 更新头部 formula-bar 上的 sheet 名
  const pane = parent.querySelector(`.excel-sheet-pane[data-sheet-index="${targetIndex}"]`);
  if (pane) {
    root.setAttribute("data-active-sheet", targetIndex);
    const formulaBar = pane.querySelector(".formula-bar");
    if (formulaBar) {
      const headerText = formulaBar.firstChild ? formulaBar.firstChild.textContent || "" : "";
      // 把 formula-bar 中第一个 · 后面的 sheet 名替换为当前 sheet
      const updated = headerText.replace(/·\s*[^\s·].*$/, `· ${pane.dataset.sheetName || ""}`);
      formulaBar.textContent = updated;
    }
  }
}

// 2026-08-04 新增：Ctrl+滚轮缩放 Excel 预览（与 PDF 一致手势）
// 作用于 <table> 元素本身 — 只缩字/行，外层容器高度不变 → 双 pane 模式下整块始终填满格子
// 范围 50%–200%，步长 10%，Ctrl+0 复位 100%
function _applyExcelZoom(article, zoom) {
  article.dataset.zoom = String(zoom);
  // 2026-08-10 修：原选择器 .excel-sheet-pane table 跟 file_preview.py 实际渲染结构不匹配
  // 真实容器是 .excel-table-wrap > table（单 sheet）/ .excel-sheet-pane > table（多 sheet tab），
  // 用 querySelectorAll 合并两者，覆盖单 sheet 和多 sheet 两种渲染路径
  const tables = article.querySelectorAll(".excel-table-wrap table, .excel-sheet-pane table");
  tables.forEach((t) => {
    t.style.zoom = String(zoom);
  });
}
...
document.addEventListener(
  ...
  (e) => {
    if (!e.ctrlKey) return;
    const article = e.target instanceof Element ? e.target.closest(".excel-preview") : null;
    if (!article) return;
    e.preventDefault();
    const current = parseFloat(article.dataset.zoom || "1");
    const delta = e.deltaY < 0 ? 0.1 : -0.1;
    const next = Math.max(0.5, Math.min(2.0, Math.round((current + delta) * 10) / 10));
    _applyExcelZoom(article, next);
  },
  { passive: false }
);
...
document.addEventListener("keydown", (e) => {
  if (!(e.ctrlKey && (e.key === "0" || e.key === "Digit0"))) return;
  const article = e.target instanceof Element ? e.target.closest(".excel-preview") : null;
  if (!article) return;
  e.preventDefault();
  _applyExcelZoom(article, 1);
});
...
function handleClick(event) {
  // 2026-08-13: 域切换/视图/筛选/period-picker 由 main 的 handleClick 处理, info 不重复处理
  if (event.target.closest("[data-domain]") ||
  ...
  // 2026-08-13: info 只处理 parse/预览独有 action，共享 action（select-file/close-viewer/mark-failed 等）交给 main
  {
    const infoOnly = new Set([
      "open-viewer", "open-xlsx-full", "show-sheet", "close-file-picker", "pick-preview-mode",
      "delete-archive-xlsx", "parse-archive", "open-parse-from-badge", "close-parse-status",
      "submit-parse", "cancel-parse", "download-parse-output",
      "preview-mode-pdf", "preview-mode-xlsx", "preview-mode-dual", "delete-parse-output",
    ]);
    if (!infoOnly.has(action.dataset.action)) return;
  }
  if (action.dataset.action === "open-viewer") {
    // 2026-08-13: 只有 cost_info 域走 info 的预览（弹选择窗口），其他域交给 main 的 handleClick（直接进详情页）
    const activeDomain = document.querySelector('.rail-button.active')?.dataset.domain;
    if (activeDomain !== "cost_info") return;
    loadArchiveDetail(action.dataset.id);
  }
  if (action.dataset.action === "close-viewer") closeViewer();
  if (action.dataset.action === "open-xlsx-full") loadXlsxFullPreviewPane(action.dataset.fullUrl);
  if (action.dataset.action === "show-sheet") switchExcelSheet(action);
  if (action.dataset.action === "close-file-picker") closeFilePickerModal();
  if (action.dataset.action === "pick-preview-mode") {
    const mode = action.dataset.mode;
    closeFilePickerModal();
    if (state.selectedArchive) {
      state.viewerPreviewMode = mode;
      if (mode === "xlsx" || mode === "dual") {
        loadXlsxPreviewPane(state.selectedArchive.archive_id).then(() => openViewer());
      } else {
        openViewer();
      }
    }
  ...
  if (action.dataset.action === "parse-archive") submitParseInBackground(action.dataset.id);
  if (action.dataset.action === "open-parse-from-badge") openParseStatusDialog(action.dataset.id);
  if (action.dataset.action === "close-parse-status") closeParseStatusDialog();
  if (action.dataset.action === "submit-parse") submitParseFromDialog(event);
  if (action.dataset.action === "cancel-parse") cancelActiveParse();
  if (action.dataset.action === "download-parse-output") downloadParseOutput();
  if (action.dataset.action === "preview-mode-pdf") { state.viewerPreviewMode = "pdf"; renderViewer(state.selectedArchive); }
  if (action.dataset.action === "preview-mode-xlsx") { state.viewerPreviewMode = "xlsx"; loadXlsxPreviewPane(action.dataset.id).then(() => renderViewer(state.selectedArchive)); }
  if (action.dataset.action === "preview-mode-dual") { state.viewerPreviewMode = "dual"; loadXlsxPreviewPane(action.dataset.id).then(() => renderViewer(state.selectedArchive)); }
  if (action.dataset.action === "delete-parse-output") deleteParseOutput();
  if (action.dataset.action === "remove-upload-file") {
    const idx = Number(action.dataset.uploadIdx);
    if (Array.isArray(state.manualUpload.files) && idx >= 0 && idx < state.manualUpload.files.length) {
      state.manualUpload.files.splice(idx, 1);
...
// 2026-08-13: 删除 info 的 quota:preview-archive/keydown/DOM初始化 等重复绑定 (main 已处理)
/* ── IIFE 闭包 + window 暴露 (只暴露 info 独有 22 个, 不覆盖 main 共享) ── */
if (typeof window !== "undefined") {
  Object.assign(window, {
    PARSE_BADGE_MAP,
    PARSE_BADGE_MIN_RUNNING_MS,
    _applyExcelZoom,
    _applyTerminalTransition,
    _parseBadgePollTimer,
    _parseTerminalTimer,
    _scheduleTerminalTransition,
    closeFilePickerModal,
    closeParseStatusDialog,
    formatDuration,
    handleParseEvent,
    isRegisterScriptArchive,
    openParseEventSource,
    openPreviewModeModal,
    previewUrl,
    renderParseBadge,
    renderParseStatusModal,
    renderPreviewModeBody,
    showParseSuccessToast,
    startParseBadgePolling,
    stopParseBadgePolling,
    switchExcelSheet,
  });
