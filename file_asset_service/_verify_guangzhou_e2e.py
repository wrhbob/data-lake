"""2026-08-07 verify: 端到端跑 1 个广州 archive,等 daemon thread 完成,实时打印事件。"""
...
ARCHIVE_ID = "1c72d368-b3db-411f-bbea-8d796ec072f0"
...
def main() -> None:
    ...
    print(f"=== submit 广州 archive={ARCHIVE_ID} ===", flush=True)
    print(f"MinerU API: {settings.mineru_api_url}", flush=True)
    print(f"extract_bucket: {settings.extract_bucket}", flush=True)
    print(flush=True)
    ...
    if job.status == "succeeded":
        ...
        print(f"✅ dropped_lines = {job.dropped_lines}", flush=True)
    else:
